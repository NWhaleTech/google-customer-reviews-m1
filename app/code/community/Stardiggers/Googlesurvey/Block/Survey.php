...
/**
 * Class Stardiggers_Googlesurvey_Block_Survey
 *
 ...
 */
class Stardiggers_Googlesurvey_Block_Survey extends Mage_Core_Block_Template
{
    ...
    const XML_PATH_OPT_IN_STYLE = 'starsurvey/opt-in/style';
    const XML_PATH_ALL_CUSTOMERS = 'starsurvey/opt-in/all_customers';
    const XML_PATH_CUSTOMER_GROUPS = 'starsurvey/opt-in/customer_groups';
    const XML_PATH_DELIVERY_TIME = 'starsurvey/general/delivery_time';
    const XML_PATH_DELIVERY_WEEKENDS = 'starsurvey/general/delivery_weekends';
    const XML_PATH_LANGUAGE = 'starsurvey/opt-in/language';
    ...
    {
        if (!Mage::helper('starsurvey')->isEnabled()) {
            return;
        }
    ...
    protected function addDataFromOrder($order)
    {
        ...
        $data = array(
            self::MERCHANT_ID => Mage::helper('starsurvey')->getMerchantId(),
    ...
    protected function getDateOffset($order, $countryCode)
    {
        ...
        $dateByRules = Mage::helper('starsurvey/customdelivery')->getConfigValue($method, $countryCode);
